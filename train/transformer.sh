# Training the Transformer model (Turkish-English) with 2 left-to-right, 2 right-to-left models
...
for i in $(seq 1 $N)
do
    mkdir -p transformer_model/ens$i
    ./marian/build/marian \
            --model transformer_model/ens$i/model.npz --type transformer \
            --train-sets data/corpus.mdnn.bpe.tr data/corpus.mdnn.bpe.en \
            --vocabs transformer_model/vocab.tren.yml transformer_model/vocab.tren.yml \
            --max-length 100 \
            ...
            --log transformer_model/ens$i/train.log --valid-log transformer_model/ens$i/valid.log \
            ...
            --devices 0 1 2 3 --sync-sgd --seed $i$i$i$i  \
...
for i in $(seq 1 $N)
do
    mkdir -p transformer_model/ens-rtl$i
    ./marian/build/marian \
            --model transformer_model/ens-rtl$i/model.npz --type transformer \
            --train-sets data/corpus.mdnn.bpe.tr data/corpus.mdnn.bpe.en \
            --vocabs transformer_model/vocab.tren.yml transformer_model/vocab.tren.yml \
            --max-length 100 \
            --mini-batch-fit -w $WORKSPACE --mini-batch 1000 --maxi-batch 1000 \
            ...
            --overwrite --keep-best \
            --early-stopping 5 --after-epochs $EPOCHS --cost-type=ce-mean-words \
            --log transformer_model/ens-rtl$i/train.log --valid-log transformer_model/ens-rtl$i/valid.log \
            --enc-depth 6 --dec-depth 6 \
            --tied-embeddings-all \
            --dim-emb 1024 --transformer-dim-ffn 4096 \
            --transformer-dropout-attention 0.1 \
...
do
    cat data/$prefix.zb.final.mdnn.bpe.tr \
        | ./marian/build/marian-decoder -c transformer_model/ens1/model.npz.best-bleu.npz.decoder.yml \
          -m transformer_model/ens?/model.npz.best-bleu.npz --quiet-translation \
          --mini-batch 16 --maxi-batch 100 --maxi-batch-sort src -w 5000 --n-best --beam-size $B \
        > data/$prefix.zb.mdnn.bpe.en.output.nbest.0

    for i in $(seq 1 $N)
    do
      ./marian/build/marian-scorer -m transformer_model/ens-rtl$i/model.npz.best-bleu.npz \
        -v transformer_model/vocab.tren.yml transformer_model/vocab.tren.yml \
...
do
    ...
    do
        cat data/$prefix.zb.final.mdnn.bpe.tr \
          | ./marian/build/marian-decoder -c transformer_model/ens$i/model.npz.best-bleu.npz.decoder.yml \
            -m transformer_model/ens$i/model.npz.best-bleu.npz --quiet-translation \
    ...
    do
        cat data/$prefix.zb.final.mdnn.bpe.tr \
          | ./marian/build/marian-decoder -c transformer_model/ens-rtl$i/model.npz.best-bleu.npz.decoder.yml \
            -m transformer_model/ens-rtl$i/model.npz.best-bleu.npz --quiet-translation \
            --mini-batch 16 --maxi-batch 100 --maxi-batch-sort src -w 5000 --beam-size $B \
          | sed 's/\@\@ //g' \
          | moses-scripts/scripts/recaser/detruecase.perl \
          | moses-scripts/scripts/tokenizer/detokenizer.perl -l en \
